...
%Decidiu-se então realizar o teste de carga com as versões mais recentes dos BPMS. Começou-se com os testes na versão 7.1.2,  a captura e análise das requisições foi realizada da mesma forma dos testes anteriores no entanto, mesmo identificando e substituindo as chaves identificadores os testes não eram executados com sucesso. De fato, algumas etapas que deveriam ser reporoduzidas não eram executadas como, por exemplo: Iniciar um processo ou iniciar uma tarefa. Isso leva crer que algumas requisições não são capturadas ou algumas etapas não são executadas de uma forma que pode ser capturada via uma requisição http, problema semelhante ao que aconteceu nos teste com a versão xx do Activiti. Executando o teste de carga utilizando o JMeter com a versão 5.19 do Activiti e ocorreram os mesmo problemas observados na versão 5.14.
% - também tentei com a versão 6.5.4 e foi pior
% - Testes funcionais funcionaram em todas...
% - colocar o teste funcional antes/inverter?
%Como os testes funcionais apresentaram sucesso em todas as versões testadas de ambas as ferramentas optou-se por verificar uma forma de realizar o teste de carga através das capturas realizadas com o Selenium. Para isso, optou-se por usar a ferramenta TestNG\footnote{TestNG. Available at:. www.testng.org} que é um framework comumente utilizado para automatizar testes com o Selenium ~\cite{bindal2014test}. O TestNG possui uma notação que permite escolher o número de vezes que o teste deve ser executado e o número de threads, assim é possível simular a execução de diversas threads simultaneamente. 
%continuar e traduzir
